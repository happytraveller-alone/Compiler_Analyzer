...
int dynamicNonTer(char* word)
{
    ...
    if (i == int(nonTerMap.size()))
    {
        ...
        {
            dynamicNum = nonTerMap[int(nonTerMap.size()) - 1].second + 1; //后面地址依次增加
            nonTerMap.push_back(make_pair(word, dynamicNum));
        }
    }
    return dynamicNum;
...
void initGrammer()
{
    ...
    while (ch != EOF)
    {
        ...
        {
            count++;
            proc[line][count] = codeNum; //将此字符存在产生式数组中，此时的line是txt中文法的line
        }
