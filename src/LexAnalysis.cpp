 ...
 *完成情况：DONE
 */
void createNewNode(char const* content, char const* descirbe, int type,
                   int addr, int line) {
 ...
 *完成情况：DONE
 */
int createNewIden(char const* content, char const* descirbe, int type,
                  int line) {
...
void printNode2() {
  ...
  while (p != NULL) {
    if (p->type == IDN) {
      ...
      filecifa << setw(10) << p->content << setw(10) << "<" << p->content
               << ",_>" << endl;
...
void printNodeLink() {
  ...
  while (p != NULL) {
    if (p->type == IDN) {
      Nodelink << setw(8) << p->content << setw(10) << p->describe << setw(10)
               << p->type << setw(8) << p->addr << setw(7) << p->line << endl;
    } else {
      Nodelink << setw(8) << p->content << setw(10) << p->describe << setw(10)
               << p->type << setw(15) << p->line << endl;
    }
    p = p->next;
  }
  Nodelink << endl;
...
void printIdentLink() {
  ...
  IdentLink << setw(8) << "内容" << setw(10) << "描述" << setw(11) << "种别码"
            << setw(10) << "地址" << setw(8) << "行号" << endl;
  while (p != NULL) {
    IdentLink << setw(8) << p->content << setw(10) << p->describe << setw(10)
              << p->type << setw(15) << p->line << endl;
    p = p->next;
  }
  IdentLink << endl;
...
void BraMappingError() {
  ...
  if (leftBig != rightBig) {
    int i =
        (leftBig > rightBig) ? (leftBig - rightBig) : (rightBig - leftSmall);
